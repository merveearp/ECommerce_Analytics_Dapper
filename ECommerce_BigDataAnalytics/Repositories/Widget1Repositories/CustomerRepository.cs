...
public class CustomerRepository(AppDbContext context) : ICustomerRepository
{
    ...
    public Task<int> GetLast7DaysCustomerCountAsync()
    {
        ...
        SELECT COUNT(*)
        FROM Customers
        WHERE CreatedDate >= DATEADD(DAY, -55, CONVERT(date, GETDATE()))
    ...
    public Task<int> GetPrevious7DaysCustomerCountAsync()
    {
        ...
        SELECT COUNT(*)
        FROM Customers
        WHERE CreatedDate >= DATEADD(DAY, -85, CONVERT(date, GETDATE()))
          AND CreatedDate <  DATEADD(DAY, -55, CONVERT(date, GETDATE()))
    ";
